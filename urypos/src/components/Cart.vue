<template>
    ...
    <div class="mt-5 flex">
        ...
        <div class="float-right rounded" v-if="this.menu.cart.length > 0">
            <button class="mr-4 rounded px-4 py-2 shadow" v-if="this.invoiceData.showUpdateButtton === true"
            ...
            <button class="rounded px-4 py-2 shadow" v-if="(this.recentOrders.invoicePrinted === 0 ||
    ...
    <div class="mt-5 border shadow" v-if="this.menu.cart.length > 0">
        <div class="cart-item-details grid w-full grid-cols-3 gap-4 md:w-full lg:w-full">
            <h3 class="ml-3 mt-2 text-base font-semibold text-gray-900 dark:text-white">
                Item Name
            </h3>
            <h3 class="ml-3 mt-2 text-center text-lg font-semibold text-gray-900 dark:text-white">
                Quantity
            </h3>
        </div>
        <div class="cart-item-details ml-3 mt-2 grid w-full grid-cols-3 gap-4 pb-2 md:w-full lg:w-full"
            v-for="(cart_item, index) in this.menu.cart" :key="index">
            <h3 class="w-full text-base text-gray-900 dark:text-white">
                {{ cart_item.item_name }}
            </h3>
            <input type="number" id="qty_input_cart" name="qty_input_cart"
                ...
                @click="this.menu.showModal(cart_item); menu.showDialogCart = true;"
                readonly
            />
    ...
    <div class="relative mt-8" v-if="this.menu.cart.length > 0">
        <label for="grand_total" class="mt-6 block text-left text-gray-900 dark:text-white">
            Grand Total
        </label>
        <input
            class="grand_total mt-3 block w-full rounded-md border bg-gray-50 p-2.5 text-sm text-gray-900 md:w-3/5 lg:w-2/5"
            :value="this.menu.grand_total || this.table.grandTotal || invoiceData.grandTotal
            " readonly
        />
        <label for="aggregatorId" class="mt-6 block text-left text-gray-900 dark:text-white"
            v-if="this.menu.selectedOrderType === 'Aggregators'">
            Aggregator ID
        </label>
        <input v-if="this.menu.selectedOrderType === 'Aggregators'" id="aggregatorId"
    ...
    <div v-if="this.invoiceData.cancelInvoiceFlag === true"
            ...
            <div class="w-full rounded-lg bg-white p-6 shadow-lg md:max-w-md">
                <div class="flex justify-end">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" @click="this.invoiceData.cancelInvoiceFlag = false">
        ...
        <div class="mt-10 flex items-center justify-center">
            <div class="w-full rounded-lg bg-white p-6 shadow-lg md:max-w-md">
                <div class="flex justify-end">
                    <span class="sr-only">Close</span>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" @click="menu.showDialogCart = false">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </div>
                ...
                <div class="relative">
                    ...
                    <input type="number" id="modeOfPayment"
                        class="mt-4 w-full appearance-none rounded border p-2 leading-tight text-gray-900 shadow focus:outline-none"
    ...
    <div class="mt-4 w-full divide-y divide-gray-200 bg-white" v-if="this.menu.cart.length > 0">
        <details>
            ...
            <div class="additional-details m-3">
                <label for="invoiceNo" class="mt-10 block text-sm font-medium text-gray-900 dark:text-white"
                ...
                <input
                    ...
                    v-if="this.table.invoiceNo || invoiceData.invoiceNumber" readonly />
                <label for="waiter" class="mt-10 block text-sm font-medium text-gray-900 dark:text-white"
                ...
                <input
                    ...
                    :class="{ hidden: this.invoiceData.waiter === '' }" :value="this.table.previousWaiter !== null &&
            ...
            " readonly />
                <label for="posProfile" class="mt-10 block text-sm font-medium text-gray-900 dark:text-white"
                ...
                <input
                    ...
                    :class="{ hidden: this.invoiceData.posProfile === '' }" v-model="this.invoiceData.posProfile"
                    readonly />
                <label for="cashier" class="mt-10 block text-sm font-medium text-gray-900 dark:text-white"
                    :class="{ hidden: this.invoiceData.cashier === '' }">
                    Cashier
                </label>
                <input
                    class="mt-3 block w-full rounded-md border bg-gray-50 p-2.5 text-sm text-gray-900 md:w-3/5 lg:w-2/5"
                    :class="{ hidden: this.invoiceData.cashier === '' }" v-model="this.invoiceData.cashier" readonly />
            </div>
        </details>
    </div>
</template>
