<template>
    <div class="flex flex-col justify-center py-20 sm:px-6 lg:px-8">
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                <form class="space-y-6" @submit.prevent="this.auth.login">
                    <div class="relative mt-1">
                        ...
                        <input name="user_id" v-model="this.auth.userId" required
                            class="block w-full appearance-none rounded-md border border-gray-300 px-3 py-2 pr-8 pl-10 placeholder-gray-400 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500 sm:text-sm"
                            placeholder="example@trilogy.com" />
                    </div>

                    <div class="mt-1">
                        <div class="relative">
                            <div class="absolute inset-y-0 right-0 flex items-center px-2">
                                <svg class="field-icon password-icon" width="20" height="20" viewBox="0 0 20 20"
                            ...
                            <input name="currentPassword" :type="this.auth.passwordFieldType"
                                v-model="this.auth.currentPassword" required placeholder="•••••"
                            ...
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 text-sm leading-5">
                                <button type="button"
                                    class="absolute inset-y-0 left-0 flex items-center pl-3 text-sm leading-5"
                                    @click="this.auth.showPassword = !this.auth.showPassword">
                                    <span v-if="this.auth.showPassword" class="text-gray-400">إخفاء</span>
                                    <span v-else class="text-gray-400">إظهار</span>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <div class="text-sm">
              <a href="#" class="font-medium text-gray-700 hover:text-blue-500">
                Forgot your password?
                    ...
                    <div>
                        <button type="submit"
                            :disabled="auth.loading || (!this.auth.currentPassword && !this.auth.userId)" :class="[
                    auth.loading || (!this.auth.currentPassword && !this.auth.userId) ? 'cursor-not-allowed' : '',
                    'flex w-full justify-center rounded-md border border-transparent bg-blue-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2'
                            ...
                            <span v-if="auth.loading" class="spinner-border spinner-border-sm">جارٍ التحقق...</span>
