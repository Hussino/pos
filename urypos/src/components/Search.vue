<template>
    ...
    <div class="mt-5 flex w-full flex-col justify-between gap-2 md:flex-row lg:mt-2">
        ...
        <div class="flex gap-2" v-if="this.menu.selectedOrderType !== 'Aggregators'">
            <div class="relative w-full">
                <label for="first" class="absolute z-50 -mt-2 ml-2 bg-white px-2 text-xs">Select Course</label>
                <select class="relative w-full rounded border border-gray-300 bg-gray-50 pt-2.5 text-sm" id="course"
                    v-model="menu.selectedCourse" @change="this.menu.displayAll = false">
                    <option v-for="(course, index) in menu.course" :key="index" :value="course.name">
                        {{ course.name }}
                    </option>
                </select>
            </div>
            <button
                ...
                type="button" :class="{ 'bg-blue-900': this.menu.priority }" @click="this.menu.showSpecialItems">
                Priority
            </button>
            <button
                class="focus:shadow-outline w-28 rounded bg-blue-700 p-2 font-bold text-white hover:bg-blue-900 focus:outline-blue-500"
